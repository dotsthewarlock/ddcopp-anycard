const JS_VERSION = window.ANYCARD_JS_VERSION || "V1.01.10";
...
const BOOKMARKLET_CODE = `javascript:(async function(){var targetUrl="https://www.anycard.ca/swap/loadcard";var targetName="anycardTargetWindow";function setStatus(message){try{var status=document.getElementById("status");if(status)status.textContent=message;}catch(err){}}function normalizedUrl(){return(location.origin+location.pathname).replace(/\/$/,"");}async function fillCurrentPage(){try{const text=await navigator.clipboard.readText();const parts=text.trim().split(/\s+/);if(parts.length<2){setStatus("No card/PIN found on clipboard.");return false;}const cardInput=parts[0];const pinInput=parts[1];const cardField=document.querySelector('input[placeholder*="Card Number"]');const pinField=document.querySelector('input[placeholder*="Card PIN"]');if(!cardField||!pinField){setStatus("AnyCard form is not ready. Open the loadcard page, then run the Bookmarklet again.");return false;}cardField.value=cardInput.trim();pinField.value=pinInput.trim();cardField.dispatchEvent(new Event('input',{bubbles:true}));pinField.dispatchEvent(new Event('input',{bubbles:true}));setTimeout(()=>{const btn=[...document.querySelectorAll('button,input[type="submit"]')].find(el=>/submit card info/i.test(el.innerText||el.value||''));if(btn) btn.click();},300);setStatus("AnyCard form filled from copied card/PIN.");return true;}catch(e){setStatus("Bookmarklet could not read the copied card/PIN or fill the form.");return false;}}try{if(normalizedUrl()===targetUrl){await fillCurrentPage();return;}var targetWindow=window.open(targetUrl,targetName);if(targetWindow&&targetWindow.focus)targetWindow.focus();setStatus("AnyCard tab/window opened or focused. Switch to https://www.anycard.ca/swap/loadcard, then click this Bookmarklet from that page.");}catch(err){setStatus("Open https://www.anycard.ca/swap/loadcard, then click this Bookmarklet from that page.");}})();`;
